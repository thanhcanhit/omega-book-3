...
//            Kiểm tra xem có phải đang nhập mã sản phẩm và đang là số không (VC_1 = 2 và VC_0 = 11)
			if (tempInput.startsWith("SP") && e.getKeyCode() >= NativeKeyEvent.VC_1
					&& e.getKeyCode() <= NativeKeyEvent.VC_0) {
				tempInput += NativeKeyEvent.getKeyText(e.getKeyCode());
			}
...
//            Add bespromotion
	...
	private void init() throws RemoteException {
		try {
			bus = (Sales_BUS) Naming.lookup(RMIService.salesBus);
...
//            Nếu số lượng mới bằng 0 thì xóa khỏi giỏ hàng
				if (newValue == 0) {
					if (JOptionPane.showConfirmDialog(this,
							"Xóa sản phẩm " + current.getProduct().getProductID() + " ra khỏi giỏ hàng",
							"Xóa sản phẩm khỏi giỏ", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
						cart.remove(current);
						renderCartTable();
						return;
					} else {
...
//                        Lấy danh mục giảm giá
						double discountAmount = bus
								.getBestProductPromotionDiscountAmountAvailable(current.getProduct().getProductID());
						if (discountAmount > 0) {
		...
		txt_orderCustomerGive.getDocument().addDocumentListener(new DocumentListener() {
			private void updateCustomerReturn() {
				try {
					...
					txt_orderCustomerReturn.setText(utilities.FormatNumber
							.toVND(Double.valueOf(Math.round(customerGive - totalAfterDiscount))));
	...
	private void renderCartTable() {
		...
		for (OrderDetail item : cart) {
			Object[] newRow = new Object[] { item.getProduct().getProductID(), item.getProduct().getName(),
					item.getQuantity(), utilities.FormatNumber.toVND(item.getPrice()),
					String.format("%.2f%%", item.getVAT()),
					utilities.FormatNumber.toVND(item.getPrice() * item.getQuantity()),
					utilities.FormatNumber.toVND(item.getSeasonalDiscount()),
					utilities.FormatNumber.toVND(item.getLineTotal()) };
...
//                    Lấy phần giảm cho 1 sản phẩm để tăng lên
					...
					Notifications.getInstance().show(Notifications.Type.INFO, "Sản phẩm này được giảm "
							+ utilities.FormatNumber.toVND(discountAmount) + " trên 1 sản phẩm");
				}
				cart.add(newLine);
				renderCartTable();
			} catch (Exception ex) {
				...
				Notifications.getInstance().show(Notifications.Type.ERROR,
						"Có lỗi xảy ra khi thêm sản phẩm " + productID);
			}
		}
	}
...
//                Lấy danh mục giảm giá
				double discountAmount = bus
						.getBestProductPromotionDiscountAmountAvailable(detail.getProduct().getProductID());
				if (discountAmount > 0) {
//                    Lấy phần giảm cho 1 sản phẩm để tăng lên
...
//        Bắt buộc nhập thông tin khách hàng trước tiên
		if (customer == null) {
			Notifications.getInstance().show(Notifications.Type.WARNING,
					"Bạn phải thêm thông tin khách hàng trước tiên");
			return;
		}
	...
	private boolean orderValidate(boolean checkCustomerGive) {
		...
		if (checkCustomerGive) {
			...
			if (isRealMoney) {
				...
				if (cmb_orderPaymentMethod.getSelectedIndex() == 0 && checkCustomerGive
						&& txt_orderCustomerGive.getText().isEmpty()) {
...
//            Kiểm tra tiền khách đưa có hợp lệ không
				if (customerGive < totalAfterDiscount) {
					Notifications.getInstance().show(Notifications.Type.WARNING,
							"Tiền khách đưa ít hơn số tiền cần thanh toán!");
					return false;
				}
			}
		}
...
//                Rerender panel
				...
				Notifications.getInstance().show(Notifications.Type.ERROR,
						"Có lỗi xảy ra khi lưu đơn hàng vào cơ sở dữ liệu" + order.getOrderID());
			}
		} catch (Exception ex) {
			Notifications.getInstance().show(Notifications.Type.ERROR,
...
//        tạo file pdf và hiển thị + in file pdf đó
	...
	private void initComponents() {
		...
		lbl_savedOrderTitle.setFont(
				lbl_savedOrderTitle.getFont().deriveFont(lbl_savedOrderTitle.getFont().getStyle() | java.awt.Font.BOLD,
						lbl_savedOrderTitle.getFont().getSize() + 6));
		...
		tbl_savedOrder.setModel(new javax.swing.table.DefaultTableModel(
				new Object[][] { { "1", "2", "3" }, { "2", "3", "5" }, { "1", "4", "6" }, { null, null, null } },
		...
		lbl_promotionList.setFont(
				lbl_promotionList.getFont().deriveFont(lbl_promotionList.getFont().getStyle() | java.awt.Font.BOLD,
						lbl_promotionList.getFont().getSize() + 6));
		...
		tbl_promotionList.setModel(new javax.swing.table.DefaultTableModel(
				new Object[][] { { null, null, null }, { null, null, null }, { null, null, null },
						{ null, null, null } },
				new String[] { "Mã khuyến mãi", "Khuyến mãi theo", "Giá trị khuyến mãi" }));
		...
		lbl_notification.setFont(
				lbl_notification.getFont().deriveFont((lbl_notification.getFont().getStyle() | java.awt.Font.ITALIC),
						lbl_notification.getFont().getSize() - 4));
		...
		pnl_customerInfo
				.setBorder(javax.swing.BorderFactory.createCompoundBorder(
						javax.swing.BorderFactory.createTitledBorder(null, "Thông tin khách hàng",
		...
		pnl_orderInfo
				.setBorder(javax.swing.BorderFactory.createCompoundBorder(
						javax.swing.BorderFactory.createTitledBorder(null, "Thông tin hóa đơn",
								javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
								javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 1, 14),
								new java.awt.Color(71, 118, 185)),
						javax.swing.BorderFactory.createEmptyBorder(5, 5, 5, 5))); // NOI18N
		...
		txt_orderPay.setEditable(false);
		txt_orderPay.setFont(txt_orderPay.getFont().deriveFont(txt_orderPay.getFont().getStyle() | java.awt.Font.BOLD,
				txt_orderPay.getFont().getSize() + 4));
		...
		pnl_orderPaymentMethod
				.setLayout(new javax.swing.BoxLayout(pnl_orderPaymentMethod, javax.swing.BoxLayout.LINE_AXIS));
		...
		pnl_orderCustomerGive
				.setLayout(new javax.swing.BoxLayout(pnl_orderCustomerGive, javax.swing.BoxLayout.LINE_AXIS));
		...
		pnl_orderCustomerReturn
				.setLayout(new javax.swing.BoxLayout(pnl_orderCustomerReturn, javax.swing.BoxLayout.LINE_AXIS));
...
//        Nếu đang không chọn dòng nào thì thông báo
		...
		if (JOptionPane.showConfirmDialog(frame_savedOrder,
				String.format("Bạn có muốn xóa đơn hàng đã lưu tạm này? (%s)", orderID), "Xóa đơn lưu tạm",
				JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
...
//        Check xem số lượng đơn tạm đã vượt quá tối đa chưa
		if (bus.getSavedOrderQuantity() >= 5) {
			Notifications.getInstance().show(Notifications.Type.WARNING,
					"Hiện đã lưu tối đa 5 đơn tạm, vui lòng xử lí các đơn lưu tạm trước đó");
			return;
		}

		if (JOptionPane.showConfirmDialog(Application.app, "Bạn có muốn lưu tạm hóa đơn này để xử lí sau không?",
				"Xác nhận lưu tạm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			...
			try {
				...
				if (isSaved) {
					Notifications.getInstance().show(Notifications.Type.SUCCESS,
							"Đã lưu tạm thành công đơn hàng " + order.getOrderID());
//                Rerender panel
					rerender();
				} else {
					Notifications.getInstance().show(Notifications.Type.ERROR,
				...
				Notifications.getInstance().show(Notifications.Type.ERROR,
						"Không thể lưu tạm hóa đơn " + order.getOrderID() + ": " + ex.getMessage());
			}
		}
	...
	private void btn_promotionSelectActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn_promotionSelectActionPerformed
		...
		try {
			...
			double discountAmount = promotion.getTypeDiscount() == DiscountType.PERCENT
					? promotion.getDiscount() / 100 * total
...
//            calculateOptionCashGive();
			Notifications.getInstance().show(Notifications.Type.SUCCESS,
					"Đã áp dụng khuyến mãi và giảm được " + utilities.FormatNumber.toVND(discountAmount));
	...
	private void renderOrderTotal() {
		...
		txt_orderPay.setText(utilities.FormatNumber.toVND(totalAfterDiscount));
		if (order.getPromotion() != null) {
			String promotionDiscountValue = order.getPromotion().getTypeDiscount() == DiscountType.PERCENT
					? order.getPromotion().getDiscount() + "%"
					: utilities.FormatNumber.toVND(order.getPromotion().getDiscount());
			lbl_notification.setText("Khuyến mãi hạng thành viên " + customer.getRank() + " được giảm: "
					+ promotionDiscountValue + " trên tổng hóa đơn");
		}
	}
	...
	private void btn_promotionActionPerformed(java.awt.event.ActionEvent evt) throws RemoteException {// GEN-FIRST:event_btn_promotionActionPerformed
		...
		if (customer.getCustomerID() == null ? defaultCustomer.getCustomerID() == null
				: customer.getCustomerID().equals(defaultCustomer.getCustomerID())) {
			Notifications.getInstance().show(Notifications.Type.INFO,
					"Khách hàng mặc định không thể sử dụng khuyến mãi trên hóa đơn");
			return;
		}
	...
	private void handleAddBest() throws RemoteException {
		...
		if (bestPromotion != null) {
			try {
				...
				double discountAmount = bestPromotion.getTypeDiscount() == DiscountType.PERCENT
						? bestPromotion.getDiscount() / 100 * total
						: bestPromotion.getDiscount();
//          Cập nhật trạng thái
				setDiscount(discountAmount);
				renderOrderTotal();
//            calculateOptionCashGive();
	...
	private void renderSavedOrderTable() throws RemoteException {
		DefaultTableModel tblModel_savedOrder = new DefaultTableModel(
			...
			/**
					 * 
					 */
			private static final long serialVersionUID = 4691613957800485075L;
...
//        Align
		...
		for (Promotion item : bus.getPromotionOfOrderAvailable(customer.getRankType().getValue())) {
			double discountAmount = item.getTypeDiscount() == DiscountType.PERCENT ? item.getDiscount() / 100 * total
					: item.getDiscount();
			String discountValue = item.getTypeDiscount() == DiscountType.PERCENT
					? String.format("%.2f%%", item.getDiscount())
					: utilities.FormatNumber.toVND(item.getDiscount());
			String discountTypeDisplay = item.getTypeDiscount() == DiscountType.PERCENT ? "Theo phần trăm (%)"
					: "Theo chiết khấu (đ)";
			tblModel_promotionList.addRow(new Object[] { item.getPromotionID(), discountTypeDisplay, discountValue,
					utilities.FormatNumber.toVND(discountAmount) });
		}
	...
	private Promotion getBestPromotionForCustomer(double total) throws RemoteException {
		...
		for (Promotion item : bus.getPromotionOfOrderAvailable(customer.getRankType().getValue())) {
			...
			double discountValue = item.getTypeDiscount() == DiscountType.PERCENT ? item.getDiscount() / 100 * total
					: item.getDiscount();
