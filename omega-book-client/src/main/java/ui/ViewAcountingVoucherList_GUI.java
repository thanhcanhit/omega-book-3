...
public class ViewAcountingVoucherList_GUI extends javax.swing.JPanel {
	...
	private ViewAccoutingVoucherList_BUS viewAcountingVoucherList_BUS;
	...
	public ViewAcountingVoucherList_GUI() throws RemoteException {
		try {
			viewAcountingVoucherList_BUS = (ViewAccoutingVoucherList_BUS) Naming
					.lookup(RMIService.viewAccountingVoucherListBus);
	...
	public void renderCustomerTable(ArrayList<AcountingVoucher> list) {
		...
		for (AcountingVoucher acountingVoucher : list) {
			...
			Object[] row = new Object[] { id, e1.getEmployeeID(), e2.getEmployeeID(), formatter.format(start),
					formatter.format(end), utilities.FormatNumber.toVND(acountingVoucher.getSale()),
					utilities.FormatNumber.toVND(acountingVoucher.getCashCountSheet().getTotal()),
					utilities.FormatNumber.toVND(acountingVoucher.getPayViaATM()),
					utilities.FormatNumber.toVND(acountingVoucher.getDifference()) };
			tblModel_acountingVoucherList.addRow(row);
		}
	}

	public void initTableModel() {
		tblModel_acountingVoucherList = new DefaultTableModel(new String[] { "Mã", "Người kiểm", "Đồng kiểm",
				"Bắt đầu", "Kết thúc", "Doanh thu", "Tiền mặt", "Thanh toán qua ATM", "Chênh lệch" }, 0);
	}
	...
	private void initComponents() {
		...
		cbo_sales.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tất cả", "Dưới 1 triệu",
		...
		cbo_difference.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tất cả", "Dưới 1 triệu",
				"Từ 1 đến 2 triệu", "Từ 2 đến 5 triệu", "Từ 5 đến 10 triệu", "Trên 10 triệu" }));
	...
	public void generatePDF(AcountingVoucher acounting) {
		...
		if (status == AcountingVoucherPrinter.PrintStatus.NOT_FOUND_FILE) {
			Notifications.getInstance().show(Notifications.Type.ERROR,
					"Lỗi không thể in phiếu kết toán: Không tìm thấy file");
		} else if (status == AcountingVoucherPrinter.PrintStatus.NOT_FOUND_PRINTER) {
			Notifications.getInstance().show(Notifications.Type.ERROR,
					"Lỗi không thể in phiếu kết toán: Không tìm thấy máy in");
		}
	}
