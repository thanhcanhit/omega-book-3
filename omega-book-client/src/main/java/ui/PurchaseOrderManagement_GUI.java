...
public final class PurchaseOrderManagement_GUI extends javax.swing.JPanel {
	...
	public final void init() throws RemoteException {
		try {
			bus = (PurchaseOrderManagement_BUS) Naming.lookup(RMIService.purchaseOrderBus);
		...
		tblModel_purchaseOrder = new DefaultTableModel(
				new String[] { "Mã đơn nhập", "Nhân viên", "Nhà cung cấp", "Ngày đặt", "Ngày nhận" }, 0);
		...
		tblModel_purchaseOrderDetail = new DefaultTableModel(
	...
	private void renderPruchaseOrdersTable(ArrayList<PurchaseOrder> purchaseOrderList) {
		...
		for (PurchaseOrder po : purchaseOrderList) {
			Object[] newRow = new Object[] { po.getPurchaseOrderID(), po.getEmployee().getName(),
					po.getSupplier().getSupplierID(), po.getOrderDate(), po.getReceiveDate() };
	...
	private void renderPurchaseOrderDetailTable(ArrayList<PurchaseOrderDetail> list) {
		...
		for (PurchaseOrderDetail pod : list) {
			...
			Object[] newRow = new Object[] { pod.getProduct().getProductID(), pod.getProduct().getName(),
					pod.getQuantity(), FormatNumber.toVND(pod.getCostPrice()), FormatNumber.toVND(pod.getLineTotal()) };
	...
	private void initComponents() {
		...
		pnl_purchaseOrderDetail.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Chi tiết đơn nhập hàng:",
		...
		pnl_purchaseOrderInfo.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Thông tin:",
				javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION,
				new java.awt.Font("Segoe UI", 1, 14), new java.awt.Color(71, 118, 185))); // NOI18N
		...
		pnl_center.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Danh sách đơn nhập hàng:",
				javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION,
				new java.awt.Font("Segoe UI", 1, 14), new java.awt.Color(71, 118, 185))); // NOI18N
	...
	private void btn_submitActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn_submitActionPerformed
		...
		if (rad_receiver.isSelected() && rad_receiver.isEnabled()) {
			if (JOptionPane.showConfirmDialog(this, "Đơn hàng này đã được nhận?", "Xác nhận đã nhận hàng",
		...
		if (rad_decline.isSelected() && rad_decline.isEnabled()) {
			if (JOptionPane.showConfirmDialog(this, "Bạn có chắc sẽ huỷ đơn hàng?", "Xác nhận huỷ đơn hàng",
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				int row = tbl_purchaseOrder.getSelectedRow();
				if (row != -1) {
					String ID = tbl_purchaseOrder.getValueAt(row, 0).toString();
