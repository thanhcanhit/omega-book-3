...
public class CreateReturnOrder_GUI extends javax.swing.JPanel {
	...
	private ReturnOrderManagement_BUS bus;
	private Bill order;
	...
	private void init() throws RemoteException {
		try {
			bus = (ReturnOrderManagement_BUS) Naming.lookup(RMIService.returnOrderBus);
		...
		cart = new ArrayList<>();
		// model
		tblModel_orderDetail = new DefaultTableModel(
				new String[] { "Mã hoá đơn", "Mã sản phẩm", "Tên sản phẩm", "Số lượng", "Đơn giá", "Tổng tiền" }, 0);
		...
		tbl_product.getModel().addTableModelListener((e) -> {
			...
			if (newValue == 0 && JOptionPane.showConfirmDialog(this,
	...
	private void renderOrderDetail(String orderID) throws RemoteException {
		...
		for (OrderDetail orderDetail1 : orderDetailList) {
			...
			String[] newRow = { orderDetail1.getOrder().getOrderID(), orderDetail1.getProduct().getProductID(),
					orderDetail1.getProduct().getName(), orderDetail1.getQuantity() + "", price + "",
					orderDetail1.getQuantity() * price + "" };
	...
	private ReturnOrder getNewValues() throws Exception {
		...
		return new ReturnOrder(returnDate, ReturnOrderStatus.PENDING, returnOrderID, employee, order, type, refund,
				cart, reason);
	}

	private void handleAddItem(String productID, int quantityOrder, double price) throws RemoteException {
	...
	private void addItemToCart(String productID, int quantityOrder, double price) throws RemoteException {
	...
	// long count = (c2.getTime().getTime() - c1.getTime().getTime()) / (24 * 3600 *
	// 1000);
	// return count > 0;
	// }

	private void createNewReturnOrder(ReturnOrder newReturnOrder) throws RemoteException {
		if (bus.createNew(newReturnOrder)) {
			if (bus.createReturnOrderDetail(newReturnOrder, cart)) {
	...
	private void initComponents() {
		...
		filler1 = new javax.swing.Box.Filler(new java.awt.Dimension(0, 0), new java.awt.Dimension(10, 10),
		...
		pnl_orderInfor
				.setBorder(javax.swing.BorderFactory.createCompoundBorder(
						javax.swing.BorderFactory.createTitledBorder(null, "Thông tin hoá đơn",
		...
		tbl_orderDetail.setModel(new javax.swing.table.DefaultTableModel(
				...
				new String[] { "Mã HĐ", "Mã SP", "Tên SP", "SL", "Đơn giá", "Tổng tiền" }) {
		...
		pnl_employeeInfor
				.setBorder(javax.swing.BorderFactory.createCompoundBorder(
						javax.swing.BorderFactory.createTitledBorder(null, "Nhân viên",
								javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
								javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 1, 14),
								new java.awt.Color(71, 118, 185)),
						javax.swing.BorderFactory.createEmptyBorder(5, 5, 5, 5))); // NOI18N
		...
		pnl_returnOrderInfor
				.setBorder(javax.swing.BorderFactory.createCompoundBorder(
						javax.swing.BorderFactory.createTitledBorder(null, "Thông tin đơn đổi trả",
								javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
								javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 1, 14),
								new java.awt.Color(71, 118, 185)),
						javax.swing.BorderFactory.createEmptyBorder(5, 5, 5, 5))); // NOI18N
	...
	private void btn_createReturnOrderActionPerformed(java.awt.event.ActionEvent evt) throws RemoteException {// GEN-FIRST:event_btn_createReturnOrderActionPerformed
		if (order == null) {
		...
		// if (checkDate()) {
		// Notifications.getInstance().show(Notifications.Type.WARNING, "Ngày đổi trả
		// không được khác ngày hiện tại");
		// return;
		// }
		...
		if (rdb_exchange.isSelected()) {
			for (ReturnOrderDetail returnOrderDetail : cart) {
				if (bus.getProduct(returnOrderDetail.getProduct().getProductID()).getInventory() < returnOrderDetail
						.getQuantity()) {
					Notifications.getInstance().show(Notifications.Type.WARNING, "Sản phẩm "
							+ returnOrderDetail.getProduct().getProductID() + " không đủ để thực hiện đổi hàng");
					return;
				}
			}
		}
	...
	private void btn_addProductActionPerformed(java.awt.event.ActionEvent evt) throws RemoteException {// GEN-FIRST:event_btn_addProductActionPerformed
	...
	private void btn_searchOrderActionPerformed(java.awt.event.ActionEvent evt) throws RemoteException {// GEN-FIRST:event_btn_searchOrderActionPerformed
		...
		if (!isAvaiable(order)) {
			Notifications.getInstance().show(Notifications.Type.WARNING,
					"Hoá đơn này không đủ điều kiện thực hiện đổi trả");
			return;
		}
	...
	private void txt_searchOrderKeyPressed(java.awt.event.KeyEvent evt) throws RemoteException {// GEN-FIRST:event_txt_searchOrderKeyPressed
		if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
			...
			if (order == null) {
				Notifications.getInstance().show(Notifications.Type.WARNING,
						"Mã hoá đơn " + orderID + " không tìm thấy");
				return;
			}
			if (bus.isExist(order)) {
				Notifications.getInstance().show(Notifications.Type.WARNING, "Hoá đơn này đã thực hiện đổi trả");
