...
public class OrderPromotionManagement_GUI extends javax.swing.JPanel implements ActionListener {
	...
	private void init() {
		...
		// model
		tblModel_promotion = new DefaultTableModel(
				new String[] { "Mã khuyến mãi", "Loại", "Giảm giá", "Hạng khách hàng", "Trạng thái" }, 0);
	...
	private boolean validPromotion() {
		...
		if (!Pattern.matches("[0-9]*", txt_discountPromo.getText())
				|| Double.parseDouble(txt_discountPromo.getText()) < 0) {
	...
	private PromotionForOrder getNewValue() throws Exception {
		...
		PromotionForOrder promotion = new PromotionForOrder(promotionID, startedDate, endedDate,
				DiscountType.fromInt(type), discount, CustomerRank.fromInt(rankCus));
		return promotion;
	}
	...
	private void initComponents() {
		...
		tbl_inforPromo.setModel(new javax.swing.table.DefaultTableModel(
				...
				new String[] { "Mã KM", "Loại KM", "Giảm giá", "Trạng thái", "Hạng khách hàng" }) {
			@SuppressWarnings("rawtypes")
		...
		cmb_rankCus.setModel(
				new javax.swing.DefaultComboBoxModel<>(new String[] { "Chưa có", "Bạc", "Vàng", "Kim cương" }));
